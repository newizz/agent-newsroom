...
## 🛠️ Tool-use policy
- **Read research files via the `Read` tool** (not `cat` / `head` / `tail` via Bash).
- **Check existence via `Glob`** (e.g. `Glob runs/<slug>/deep-research/mindmap.json`) — not `[ -f ... ]`.
- **Write HTML via `Write` tool** (not `echo > file`).
- **Edit existing files via `Edit` tool** (not `sed -i`).
- **Bash only for:** `./scripts/deploy.sh`, `git`, `mkdir`, and copying `infographic.png` to dashboard dir.
...
After reading `brief.md`, look for a `**Language:**` field:

- If the field is **absent or set to `en`** → build the dashboard in **English** (default).
- If the field starts with `**Language:** th` → build the dashboard in **Thai**:
  1. Apply the Thai UI strings table below to every piece of structural text you write yourself (section headers, nav labels, button text, reel labels, footer, callout labels, etc.).
  2. **Translate research content into Thai** when rendering it into the dashboard — insight reel slides, fact cards, case study descriptions, section prose, glossary entries, and any other text you pull from `research.md` or `deep-research/summary.md`. (Exception: proper nouns, brand names, and acronyms like "Nash equilibrium", "WEF", "OECD" may be kept in English with a Thai explanation alongside.)
  3. **`meta.json` must also be in Thai** — `title` and `subtitle` in Thai, `tags` as Thai keywords (plus `"thai"` as the last tag so the library can filter by language).

### Thai UI strings table

Use these exact strings when building in Thai mode:

| English (default) | Thai |
|---|---|
| Key findings | ผลการค้นพบสำคัญ |
| Sources | แหล่งข้อมูล |
| Back to office | กลับหน้าหลัก |
| Generated by agent-newsroom | สร้างโดย agent-newsroom |
| Deep Research — synthesized from N sources via NotebookLM | การวิจัยเชิงลึก — สังเคราะห์จาก N แหล่งข้อมูลผ่าน NotebookLM |
| Visual overview | ภาพรวม |
| Generated by NotebookLM during deep research. | สร้างโดย NotebookLM ระหว่างการวิจัยเชิงลึก |
| Mind map | แผนที่ความคิด |
| Mind map generated from research synthesis (NotebookLM mind map unavailable for this run). | แผนที่ความคิดสร้างจากการสังเคราะห์งานวิจัย (NotebookLM ไม่มีแผนที่ความคิดสำหรับรอบนี้) |
| Read more | อ่านเพิ่มเติม |
| Jump to section | ไปที่หัวข้อ |
...
| How to develop | วิธีพัฒนา |
| Real-world example | ตัวอย่างในชีวิตจริง |
| Loading… | กำลังโหลด… |
| Previous | ก่อนหน้า |
| Next | ถัดไป |
| Video sources | แหล่งข้อมูลวิดีโอ |

If you need a string not in this table, write it in Thai. When in doubt, prefer plain, clear Thai over formal academic phrasing.
...
**ALWAYS embed a mind map section in deep mode** — even if NotebookLM didn't produce one. Use this 3-tier fallback:

#### Tier 1: NotebookLM mindmap.json (preferred)

If `runs/<slug>/deep-research/mindmap.json` exists AND `mind_map` field is non-null:

1. Inspect the JSON structure (NotebookLM nests as `{ title, children: [...] }`)
2. Convert to markmap markdown format

#### Tier 2: Generate from research.md (fallback)

If `mindmap.json` is missing OR has `"mind_map": null` (common when NotebookLM has thin content):

**Generate the markmap markdown YOURSELF** from `runs/<slug>/research.md` + `runs/<slug>/deep-research/summary.md`. Use this structure:

```markdown
# <Topic title from brief>

## TL;DR
- <first sentence of TL;DR>
- <second sentence>
...
## Deep dive
### <H2/H3 heading from research.md>
- <one bullet per main point>
### <next section>
- <points>
...
## Sources
- <Top 3-5 source titles, shortened>
```

Rules for the fallback markdown:
- ≤ 80 lines total (markmap gets crowded otherwise)
- Each bullet ≤ 12 words
- Use research.md headings as branches (depth 2-3)
- Include numeric facts under "Data" branch
- End with "Sources" branch listing top 3-5

Mark the section with a small footnote so the user knows it's a fallback:
```html
<div class="text-xs muted mt-2">Mind map generated from research synthesis (NotebookLM mind map unavailable for this run).</div>
```

#### Tier 3: Skip section entirely (only if Tier 1+2 both fail)

If you have neither valid mindmap.json NOR research.md (impossible in deep mode but possible in edge cases) → omit the section.

#### Embedding (both tiers use the same markup)
...
<section class="max-w-5xl mx-auto px-6 mb-12">
  ...
  <div class="surface rounded-xl p-4" style="height: 520px;">
...
- point
  ...
  <!-- include the fallback footnote ONLY if Tier 2 was used -->
</section>
...
The markdown inside `<script type="text/template">` is what you write — either converted from mindmap.json OR generated from research.md.
